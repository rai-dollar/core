...
contract TellorCaller is ITellorCaller {
    ...
    */
        ...
        )
    {
        ...
        // If no reported values, bail out immediately
        if (_count == 0) return (false, 0, 0);

        uint256 _time =
            tellor.getTimestampbyRequestIDandIndex(_requestId, _count - 1);
        uint256 _value = tellor.retrieveData(_requestId, _time);

        // Return true if we successfully retrieved data (timestamp > 0 indicates valid data)
        if (_time > 0) return (true, _value, _time);
        return (false, 0, 0);
    }
}
