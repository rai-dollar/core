...
library BufferHelpers {
     ...
     */
    function getBufferUnderlyingImbalance(
        ...
        if (bufferBalance.getBalanceDerived() > 0) {
            ...
            wrappedBalanceAsUnderlying = wrappedToken
                .previewMint(bufferBalance.getBalanceDerived())
                .toInt256();
        }
     ...
     * These balances are equal value, given the rate.
     */
    function getBufferWrappedImbalance(
        bytes32 bufferBalance,
        IERC4626 wrappedToken
    ) internal view returns (int256) {
        ...
        if (bufferBalance.getBalanceRaw() > 0) {
            ...
            underlyingBalanceAsWrapped = wrappedToken
                .previewWithdraw(bufferBalance.getBalanceRaw())
                .toInt256();
        }
