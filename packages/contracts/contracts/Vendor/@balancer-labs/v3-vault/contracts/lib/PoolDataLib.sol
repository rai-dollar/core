...
import {PoolData, TokenInfo, TokenType, Rounding} from "../../../v3-interfaces/contracts/vault/VaultTypes.sol";
...
library PoolDataLib {
    ...
    function load(
        ...
        poolData.decimalScalingFactors = PoolConfigLib.getDecimalScalingFactors(
            poolData.poolConfigBits,
            numTokens
        );
        ...
        for (uint256 i = 0; i < numTokens; ++i) {
            ...
            bool tokenSubjectToYieldFees = tokenInfo.paysYieldFees &&
                tokenInfo.tokenType == TokenType.WITH_RATE;

            // Do not charge yield fees before the pool is initialized, or in recovery mode.
            if (tokenSubjectToYieldFees) {
                uint256 aggregateYieldFeePercentage = poolData
                    .poolConfigBits
                ...
                if (aggregateYieldFeeAmountRaw > 0) {
                    updateRawAndLiveBalance(
                        poolData,
                        i,
                        balanceRaw - aggregateYieldFeeAmountRaw,
                        roundingDirection
                    );
                }
            }
        ...
        for (uint256 i = 0; i < numTokens; ++i) {
            ...
            if (storedBalanceRaw > poolData.balancesRaw[i]) {
                ...
                poolAggregateProtocolFeeAmounts[token] = packedProtocolFeeAmounts.setBalanceDerived(
                    packedProtocolFeeAmounts.getBalanceDerived() +
                        (storedBalanceRaw - poolData.balancesRaw[i])
                );
            }
    ...
    function _computeYieldFeesDue(
        ...
        if (currentLiveBalance > lastLiveBalance) {
            unchecked {
                ...
                uint256 aggregateYieldFeeAmountScaled18 = (currentLiveBalance - lastLiveBalance)
                    .mulUp(aggregateYieldFeePercentage);
