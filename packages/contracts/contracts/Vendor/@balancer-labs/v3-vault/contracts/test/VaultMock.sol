...
import {IProtocolFeeController} from "../../../v3-interfaces/contracts/vault/IProtocolFeeController.sol";
import {IRateProvider} from "../../../v3-interfaces/contracts/solidity-utils/helpers/IRateProvider.sol";
import {IVaultExtension} from "../../../v3-interfaces/contracts/vault/IVaultExtension.sol";
import {IVaultMainMock} from "../../../v3-interfaces/contracts/test/IVaultMainMock.sol";
import {IAuthorizer} from "../../../v3-interfaces/contracts/vault/IAuthorizer.sol";
import {IVaultAdmin} from "../../../v3-interfaces/contracts/vault/IVaultAdmin.sol";
import {IVault} from "../../../v3-interfaces/contracts/vault/IVault.sol";
import {IHooks} from "../../../v3-interfaces/contracts/vault/IHooks.sol";
import "../../../v3-interfaces/contracts/vault/VaultTypes.sol";

import {StorageSlotExtension} from "../../../v3-solidity-utils/contracts/openzeppelin/StorageSlotExtension.sol";
import {PackedTokenBalance} from "../../../v3-solidity-utils/contracts/helpers/PackedTokenBalance.sol";
import {BufferHelpers} from "../../../v3-solidity-utils/contracts/helpers/BufferHelpers.sol";
import {TransientStorageHelpers, TokenDeltaMappingSlotType} from "../../../v3-solidity-utils/contracts/helpers/TransientStorageHelpers.sol";
import {WordCodec} from "../../../v3-solidity-utils/contracts/helpers/WordCodec.sol";
...
contract VaultMock is IVaultMainMock, Vault {
    ...
    function manualSetVaultState(bool isVaultPaused, bool isQueryDisabled) public {
        _vaultStateBits = _vaultStateBits.setVaultPaused(isVaultPaused).setQueryDisabled(
            isQueryDisabled
        );
    }
    ...
    function manualSetPoolConfig(address pool, PoolConfig memory config) public {
        ...
        poolConfigBits = _manualSetAggregateSwapFeePercentage(
            poolConfigBits,
            config.aggregateSwapFeePercentage
        );
        poolConfigBits = poolConfigBits.setAggregateYieldFeePercentage(
        ...
        poolConfigBits = poolConfigBits.setAddLiquidityCustom(
            config.liquidityManagement.enableAddLiquidityCustom
        );
    ...
    function manualSetHooksConfig(address pool, HooksConfig memory hooksConfig) public {
        ...
        poolConfigBits = poolConfigBits.setHookAdjustedAmounts(
            hooksConfig.enableHookAdjustedAmounts
        );
        poolConfigBits = poolConfigBits.setShouldCallBeforeInitialize(
            hooksConfig.shouldCallBeforeInitialize
        );
        poolConfigBits = poolConfigBits.setShouldCallAfterInitialize(
        ...
        poolConfigBits = poolConfigBits.setShouldCallBeforeAddLiquidity(
            hooksConfig.shouldCallBeforeAddLiquidity
        );
        poolConfigBits = poolConfigBits.setShouldCallAfterAddLiquidity(
    ...
    function manualSetPoolTokenInfo(
        address pool,
        IERC20[] memory tokens,
    ...
    function manualSetPoolTokensAndBalances(
        ...
        require(
            tokens.length == tokenBalanceLiveScaled18.length,
            "VaultMock: TOKENS_LENGTH_MISMATCH"
        );

        mapping(uint256 tokenIndex => bytes32 packedTokenBalance)
            storage poolTokenBalances = _poolTokenBalances[pool];
        for (uint256 i = 0; i < tokens.length; ++i) {
            poolTokenBalances[i] = PackedTokenBalance.toPackedBalance(
                tokenBalanceRaw[i],
                tokenBalanceLiveScaled18[i]
            );
        }
    ...
    function ensureUnpausedAndGetVaultState(
        address pool
    ) public view returns (VaultState memory vaultState) {
    ...
    function computeYieldFeesDue(
        ...
        return
            PoolDataLib._computeYieldFeesDue(
                poolData,
                lastLiveBalance,
                tokenIndex,
    ...
    function getLastLiveBalances(
        address pool
    ) external view returns (uint256[] memory lastBalancesLiveScaled18) {
        mapping(uint256 tokenIndex => bytes32 packedTokenBalance)
            storage poolTokenBalances = _poolTokenBalances[pool];
    ...
    {
        PoolSwapParams memory poolSwapParams = _buildPoolSwapParams(
            vaultSwapParams,
            state,
            poolData
        );
    ...
    function manualGetAggregateSwapFeeAmount(
        address pool,
        IERC20 token
    ...
    function manualGetAggregateYieldFeeAmount(
        address pool,
        IERC20 token
    ) external view returns (uint256) {
    ...
    function manualSetAggregateYieldFeeAmount(address pool, IERC20 token, uint256 value) external {
        _aggregateFeeAmounts[pool][token] = _aggregateFeeAmounts[pool][token].setBalanceDerived(
            value
        );
    }
    ...
    function internalGetBufferUnderlyingImbalance(
    ...
    function internalGetBufferWrappedImbalance(
        IERC4626 wrappedToken
    ) external view returns (int256) {
        bytes32 bufferBalance = _bufferTokenBalances[wrappedToken];
    ...
    function manualGetIsUnlocked()
        external
        view
        returns (StorageSlotExtension.BooleanSlotType slot)
    {
        return _isUnlocked();
    }

    function manualGetNonzeroDeltaCount()
        external
        view
        returns (StorageSlotExtension.Uint256SlotType slot)
    {
    ...
    function manualSetBufferOwnerShares(
        IERC4626 wrappedToken,
        address owner,
        uint256 shares
    ) external {
    ...
    function manualSetBufferBalances(
        IERC4626 wrappedToken,
        uint256 underlyingAmount,
        uint256 wrappedAmount
    ) external {
        _bufferTokenBalances[wrappedToken] = PackedTokenBalance.toPackedBalance(
            underlyingAmount,
            wrappedAmount
        );
    }
    ...
    function manualGetAddLiquidityCalledFlagBySession(
        address pool,
        uint256 sessionId
    ...
    function manualUpdateAggregateSwapFeePercentage(
        address pool,
    ...
    ) external returns (uint256 amountOutWrapped) {
    ...
    ) external returns (uint256 amountOutUnderlying) {
    ...
    ) external returns (uint256 amountInWrapped) {
