...
import {Rounding} from "../../v3-interfaces/contracts/vault/VaultTypes.sol";
...
import {Authentication} from "../../v3-solidity-utils/contracts/helpers/Authentication.sol";
...
contract VaultAdmin is IVaultAdmin, VaultCommon, Authentication, VaultGuard {
    ...
    *******************************************************************************/
        ...
        returns (uint256[] memory totalSwapFees, uint256[] memory totalYieldFees)
    {
        ...
        for (uint256 i = 0; i < poolTokens.length; ++i) {
            ...
            (totalSwapFees[i], totalYieldFees[i]) = _aggregateFeeAmounts[pool][token]
                .fromPackedBalance();
    ...
    function updateAggregateSwapFeePercentage(
        ...
        onlyProtocolFeeController
    {
        _poolConfigBits[pool] = _poolConfigBits[pool].setAggregateSwapFeePercentage(
    ...
    function updateAggregateYieldFeePercentage(
        ...
        onlyProtocolFeeController
    {
        _poolConfigBits[pool] = _poolConfigBits[pool].setAggregateYieldFeePercentage(
    ...
    *******************************************************************************/
    ...
    function disableRecoveryMode(
        address pool
    ) external onlyVaultDelegateCall withRegisteredPool(pool) authenticate {
    ...
    *******************************************************************************/
        ...
        returns (uint256 issuedShares)
    {
        ...
        bytes32 bufferBalances = PackedTokenBalance.toPackedBalance(
            amountUnderlyingRaw,
            amountWrappedRaw
        );
        ...
        returns (uint256 amountUnderlyingRaw, uint256 amountWrappedRaw)
    {
        ...
        amountUnderlyingRaw = bufferBalances.getBalanceRaw().mulDivUp(
            exactSharesToIssue,
            totalShares
        );
        amountWrappedRaw = bufferBalances.getBalanceDerived().mulDivUp(
            exactSharesToIssue,
            totalShares
        );
                ...
                _vault.unlock(
                    abi.encodeCall(
                        ...
                        (
                            wrappedToken,
                            sharesToRemove,
                            minAmountUnderlyingOutRaw,
                            minAmountWrappedOutRaw,
                            msg.sender
                        )
                    )
                ),
        ...
        returns (uint256 removedUnderlyingBalanceRaw, uint256 removedWrappedBalanceRaw)
    {
        ...
        removedUnderlyingBalanceRaw =
            (bufferBalances.getBalanceRaw() * sharesToRemove) /
            totalShares;
        removedWrappedBalanceRaw =
            (bufferBalances.getBalanceDerived() * sharesToRemove) /
            totalShares;
    ...
    function _queryModeBufferSharesIncrease(
        IERC4626 wrappedToken,
        address to,
        uint256 amount
    ) internal {
    ...
    function getBufferBalance(
        IERC4626 token
    ) external view onlyVaultDelegateCall returns (uint256, uint256) {
        // The first balance is underlying, and the last is wrapped balance.
        return (
            _bufferTokenBalances[token].getBalanceRaw(),
            _bufferTokenBalances[token].getBalanceDerived()
        );
    }
