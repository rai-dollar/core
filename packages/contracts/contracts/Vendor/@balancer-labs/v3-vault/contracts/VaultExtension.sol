...
import {EVMCallModeHelpers} from "../../v3-solidity-utils/contracts/helpers/EVMCallModeHelpers.sol";
...
import {RevertCodec} from "../../v3-solidity-utils/contracts/helpers/RevertCodec.sol";
...
contract VaultExtension is IVaultExtension, VaultCommon, Proxy {
    ...
    *******************************************************************************/
    ...
    function getAddLiquidityCalledFlag(
        address pool
    ) external view onlyVaultDelegateCall returns (bool) {
    ...
    *******************************************************************************/
    ...
    function _registerPool(address pool, PoolRegistrationParams memory params) internal {
        ...
        // Store the configuration, and mark the pool as registered.
        {
            ...
            ) = _protocolFeeController.registerPool(
                    pool,
                    params.roleAccounts.poolCreator,
                    params.protocolFeeExempt
                );
            ...
            poolConfigBits = poolConfigBits.setTokenDecimalDiffs(
            ...
            poolConfigBits = poolConfigBits.setAggregateSwapFeePercentage(
            ...
            if (params.poolHooksContract != address(0)) {
                ...
                poolConfigBits = poolConfigBits.setShouldCallBeforeSwap(
        ...
        returns (uint256 bptAmountOut)
    {
        ...
        // favoring the pool.
        uint256[] memory exactAmountsInScaled18 = exactAmountsIn
        ...
        if (poolData.poolConfigBits.shouldCallBeforeInitialize()) {
            HooksConfigLib.callBeforeInitializeHook(
        ...
        if (poolData.poolConfigBits.shouldCallAfterInitialize()) {
            ...
            HooksConfigLib.callAfterInitializeHook(
                exactAmountsInScaled18,
                bptAmountOut,
                userData,
                hooksContract
    ...
    function _initialize(
        ...
        mapping(uint256 tokenIndex => bytes32 packedTokenBalance)
            storage poolBalances = _poolTokenBalances[pool];
    ...
    *******************************************************************************/
    ...
    /// @inheritdoc IVaultExtension
    function isERC4626BufferInitialized(
    ...
    /// @inheritdoc IVaultExtension
    function getERC4626BufferAsset(
        IERC4626 wrappedToken
    ) external view onlyVaultDelegateCall returns (address asset) {
    ...
    *******************************************************************************/
    ...
    /// @inheritdoc IVaultExtension
    function getProtocolFeeController()
        external
        view
        onlyVaultDelegateCall
    ...
    *******************************************************************************/
    ...
    function quote(
        bytes calldata data
    ) external query onlyVaultDelegateCall returns (bytes memory result) {
