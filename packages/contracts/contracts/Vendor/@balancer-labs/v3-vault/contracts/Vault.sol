...
import {Proxy} from "../../dependencies/@openzeppelin/contracts/proxy/Proxy.sol";
...
import {IPoolLiquidity} from "../../v3-interfaces/contracts/vault/IPoolLiquidity.sol";
...
import {IVaultMain} from "../../v3-interfaces/contracts/vault/IVaultMain.sol";
import {IBasePool} from "../../v3-interfaces/contracts/vault/IBasePool.sol";
...
import {ScalingHelpers} from "../../v3-solidity-utils/contracts/helpers/ScalingHelpers.sol";
import {CastingHelpers} from "../../v3-solidity-utils/contracts/helpers/CastingHelpers.sol";
import {BufferHelpers} from "../../v3-solidity-utils/contracts/helpers/BufferHelpers.sol";
import {InputHelpers} from "../../v3-solidity-utils/contracts/helpers/InputHelpers.sol";
import {FixedPoint} from "../../v3-solidity-utils/contracts/math/FixedPoint.sol";
import {TransientStorageHelpers} from "../../v3-solidity-utils/contracts/helpers/TransientStorageHelpers.sol";
...
contract Vault is IVaultMain, VaultCommon, Proxy {
    ...
    *******************************************************************************/
        ...
        returns (uint256 amountCalculated, uint256 amountIn, uint256 amountOut)
    {
        ...
        if (poolData.poolConfigBits.shouldCallBeforeSwap()) {
            ...
            swapState.amountGivenScaled18 = _computeAmountGivenScaled18(
                vaultSwapParams,
                poolData,
                swapState
            );
        ...
        )
    {
        ...
        if (vaultSwapParams.kind == SwapKind.EXACT_IN) {
            ...
            locals.totalSwapFeeAmountScaled18 = poolSwapParams.amountGivenScaled18.mulUp(
                swapState.swapFeePercentage
            );
        ...
        mapping(uint256 tokenIndex => bytes32 packedTokenBalance)
            storage poolBalances = _poolTokenBalances[vaultSwapParams.pool];
    ...
    ***************************************************************************/
        ...
        returns (uint256[] memory amountsIn, uint256 bptAmountOut, bytes memory returnData)
    {
        ...
        uint256[] memory maxAmountsInScaled18 = params
            .maxAmountsIn
        ...
        )
    {
        ...
        if (params.kind == AddLiquidityKind.PROPORTIONAL) {
            ...
            // Uses msg.sender as the Router (the contract that called the Vault).
            (amountsInScaled18, bptAmountOut, swapFeeAmounts, returnData) = IPoolLiquidity(
                params.pool
        ...
        for (uint256 i = 0; i < locals.numTokens; ++i) {
            ...
            (
                swapFeeAmounts[i],
                locals.aggregateSwapFeeAmountRaw
            ) = _computeAndChargeAggregateSwapFees(
                poolData,
                swapFeeAmounts[i],
                params.pool,
    ...
    ***************************************************************************/
        ...
        returns (uint256 bptAmountIn, uint256[] memory amountsOut, bytes memory returnData)
    {
        ...
        uint256[] memory minAmountsOutScaled18 = params
            .minAmountsOut
        ...
        )
    {
        ...
        if (params.kind == RemoveLiquidityKind.PROPORTIONAL) {
            ...
            // Uses msg.sender as the Router (the contract that called the Vault).
            (bptAmountIn, amountsOutScaled18, swapFeeAmounts, returnData) = IPoolLiquidity(
                params.pool
    ...
    function _computeAndChargeAggregateSwapFees(
        ...
        if (totalSwapFeeAmountScaled18 > 0) {
            ...
            if (poolData.poolConfigBits.isPoolInRecoveryMode() == false) {
                uint256 aggregateSwapFeePercentage = poolData
                    .poolConfigBits
                ...
                aggregateSwapFeeAmountRaw = totalSwapFeeAmountRaw.mulDown(
        ...
        returns (uint256 amountInUnderlying, uint256 amountOutWrapped, bytes32 bufferBalances)
    {
        if (kind == SwapKind.EXACT_IN) {
            ...
            (amountInUnderlying, amountOutWrapped) = (
                amountGiven,
                wrappedToken.previewDeposit(amountGiven - 1) - 1
            );
        } else {
            ...
            // preview result to compensate for any rounding imprecision, so that the buffer does not leak value.
            (amountInUnderlying, amountOutWrapped) = (
                wrappedToken.previewMint(amountGiven + 1) + 1,
                amountGiven
            );
        }
        ...
        if (bufferBalances.getBalanceDerived() >= amountOutWrapped) {
            ...
            if (kind == SwapKind.EXACT_IN) {
                ...
                int256 bufferUnderlyingImbalance = bufferBalances.getBufferUnderlyingImbalance(
                    wrappedToken
                );
                vaultUnderlyingDeltaHint = (amountInUnderlying.toInt256() +
                    bufferUnderlyingImbalance).toUint256();
                underlyingToken.forceApprove(address(wrappedToken), vaultUnderlyingDeltaHint);
                vaultWrappedDeltaHint = wrappedToken.deposit(
                ...
                // (i.e. `bufferBalances.getBalanceDerived() >= amountOutWrapped`).
                int256 bufferWrappedImbalance = bufferBalances.getBufferWrappedImbalance(
                    wrappedToken
                );
                vaultWrappedDeltaHint = (amountOutWrapped.toInt256() - bufferWrappedImbalance)
                    .toUint256();
        ...
        returns (uint256 amountInWrapped, uint256 amountOutUnderlying, bytes32 bufferBalances)
    {
        if (kind == SwapKind.EXACT_IN) {
            ...
            (amountInWrapped, amountOutUnderlying) = (
                amountGiven,
                wrappedToken.previewRedeem(amountGiven - 1) - 1
            );
        } else {
            ...
            (amountInWrapped, amountOutUnderlying) = (
                wrappedToken.previewWithdraw(amountGiven + 1) + 1,
                amountGiven
            );
        }
        ...
        if (bufferBalances.getBalanceRaw() >= amountOutUnderlying) {
            ...
            if (kind == SwapKind.EXACT_IN) {
                ...
                vaultWrappedDeltaHint = (amountInWrapped.toInt256() + bufferWrappedImbalance)
                    .toUint256();
                vaultUnderlyingDeltaHint = wrappedToken.redeem(
                    vaultWrappedDeltaHint,
                    address(this),
                    address(this)
                );
                ...
                vaultUnderlyingDeltaHint = (amountOutUnderlying.toInt256() -
                    bufferUnderlyingImbalance).toUint256();
                vaultWrappedDeltaHint = wrappedToken.withdraw(
                    vaultUnderlyingDeltaHint,
                    address(this),
                    address(this)
                );
